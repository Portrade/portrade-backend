...
public class QnaController {
    ...
    public ResponseEntity<IdResponseDto> createAnswerApi(
            @Parameter(description = "답변할 게시글 ID") @PathVariable("qnaId") Long qnaId,
    ...
    public ResponseEntity<QnasResponseDto> getQnasApi(
            ...
            @Parameter(description = "조회할 1:1 문의 게시글 상태") @RequestParam(value = "type", defaultValue = "all") String type) {
        QnasResponseDto qnasResponseDto = qnaService.getQnas(page, size, keyword, type);
    ...
    public ResponseEntity<QnaDetailResponseDto> getQnaDetailApi(
            @Parameter(description = "조회할 게시글 ID") @PathVariable("qnaId") Long qnaId,
            @Parameter(hidden = true) @CurrentUser User user) {
    ...
    @DeleteMapping("/{qnaId}")
    public ResponseEntity<Void> deleteQnaApi(
            @Parameter(description = "삭제할 게시글 ID") @PathVariable("qnaId") Long qnaId,
